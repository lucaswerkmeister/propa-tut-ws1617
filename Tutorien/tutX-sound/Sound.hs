{- Musterloesung zur Aufgabe ,,Musik'' -}
module Sound where

import System.Process -- Dieser Import wird nur fuer den playWave hack am Ende noetig.
                      -- Dieser war aber nicht Teil der eigentlichen Aufgabe!
import Wave
...
-- Antwort auf die Frage was es fuer ein Song ist:
-- Popcorn von Gershon Kingsley
-- einer der Pioniere beim Einsatz von Synthesizern in der Musik
-- Er benutzte den von Dr. Robert Moog entwickelten Moog Synthesizer.
--Ein paar Bilder gibts hier:
--  https://www.moogmusic.com/legacy/moog-product-timeline
--
-- (der Song erreichte 1971 Platze 1 der Charts in einer Version von
-- ,,Hot Butter'' und wurde noch oft gecovert. Die letzte bekannte Version ist
-- wohl die vom ,,Crazy Frog'' aus dem Jahr 2007 (leider...)

-- The following makes testing on linux easier...
playWave signal = do
  writeWave "/tmp/t.wav" signal
  callCommand "aplay /tmp/t.wav"
