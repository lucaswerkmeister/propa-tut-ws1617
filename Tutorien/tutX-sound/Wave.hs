...
writeToHandle handle signal = wth signal Nothing 0
  where
    wth [] err length | err `seq` length `seq` False = undefined
                      | otherwise = return (err,length)
    wth (x:xs) err length | err `seq` length `seq` False = undefined
                          | otherwise = do
...
writeWave filename (Signal signal) = do
  ...
  IO.writeFile (filename ++ ".html") ("<link rel='stylesheet' href='/basic.css'/><audio controls='controls' src='" ++ filename ++ "' style='position:absolute;left:50%;top:50%;transform:translateX(-50%)translateY(-50%)'>")
